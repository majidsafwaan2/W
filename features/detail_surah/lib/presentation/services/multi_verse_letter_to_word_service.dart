...
import 'package:detail_surah/presentation/services/letter_to_word_detector_service.dart';
...
class MultiVerseLetterToWordRecitationResult {
  ...
  MultiVerseLetterToWordRecitationResult({
...
class MultiVerseLetterToWordRecitationService {
  /// Detect which verses were recited using letter-level checking but word-level results
  static MultiVerseLetterToWordRecitationResult detectMultiVerseRecitationLetterBasedWordLevel(
    ...
    for (int verseIndex = 0; verseIndex < allVerses.length; verseIndex++) {
      ...
      // If we matched at least 30% of the verse words, consider it recited
      ...
      if (matchRatio >= 0.3 && tempTranscriptionIndex > transcriptionIndex) {
        ...
        // Detect mistakes using letter-level checking but word-level results
        final result = LetterToWordDetectorService.detectMistakesLetterBasedWordLevel(
          verse.text.arab,
          verseTranscription,
        );
    ...
    return MultiVerseLetterToWordRecitationResult(
